		...
		<div class='container'>
			...
			@if(isset($svc))
				<form class='contact-form' name='cadastro' id='cadastro' method='post' action='{{url("adm/service/$svc->cdServico")}}' enctype='multiform/form-data'>
					<!-- passo pela url o id do servico em edicao -->
				@method('PUT')
			@else
                                ...
                                <div class='col-md-11 col-xs-12'>
                                    ...
                                    <select name='select_employee[]' id='select_employee'>
                                        ...
                                        @foreach($employees as $emp)
                                            <script>
                                                var values = [];
												// sessionStorage = armazena informacoes durante o periodo de uma secao
							...
							@if(isset($rel))
									...
									<script type='text/javascript'>
										function createFields(employee) {
												...
												$('<img>').attr({class: 'removeFromTable', src: 'http://localhost/recantodabeleza/laravel/public/img/icons/deleteEmployee.png'}).click(removeItem).appendTo(div);
												div.appendTo(row);
