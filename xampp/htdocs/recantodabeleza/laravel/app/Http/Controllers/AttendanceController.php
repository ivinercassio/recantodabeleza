...
class AttendanceController extends Controller
{
    ...
    protected function getUnpaidAttendances(Request $request){
        $att = $this->objAttendance->where('situacao', 'N')->where('cdCliente', $request->client)->get()->last();
        $resultado = new ModelAttendance();
        if($att != NULL){
            // caso haja pagamentos pendentes
            $resultado = $this->objPayment->where('situacao', 'N')->where('cdAtendimento', $att->cdAtendimento)->get();
        }
        return $resultado;              
    }
    ...
    protected function quitarAtendimento(Request $request){ 
        $request->cdCliente = intval($request->cdCliente);

        // verifico se ainda ha atendimentos nao pagos
        $att = $this->objAttendance->where('situacao', 'N')->where('cdCliente', $request->cdCliente)->get()->last();

        if($att != NULL){
            // vetor com todo as parcelas nao pagas
            $pagamento = $this->objPayment->where('situacao', 'N')->where('cdAtendimento', $att->cdAtendimento)->get();
            // caso esse vetor esteja vazio --> mudar situacao do atendimento para pago
            if($pagamento->get('items') == null){
                $this->objAttendance->where('cdAtendimento', $att->cdAtendimento)->update(['situacao' => 'P']);
            }
            return;
        }
    }
